...
def find_nearest_point(ego_x, ego_y, x_list, y_list):
    ...
    for i in range(len(x_list)):
        temp_dist = calc_dist(x_list[i], y_list[i], ego_x, ego_y)
...
def calc_error(ego_x, ego_y, ego_yaw, x_list, y_list, wpt_look_ahead=0):
    ...
    # 4. Calculate errors
    wp_del_x = x_list[near_ind+1] - x_list[near_ind]
    wp_del_y = y_list[near_ind+1] - y_list[near_ind]

    error_yaw = np.tan(wp_del_y/wp_del_x)

    error_yaw = normalize_angle(error_yaw) # Normalize angle to [-pi, +pi]
    error_y   = local_y_list[near_ind+1] 
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        # Publish command
        # while(1):
        t_cmd = raw_input("enter throttle\n")
        s_cmd = raw_input("enter steering\n")
        if t_cmd == 'w':
            wpt_control.throttle_cmd = wpt_control.throttle_cmd + 0.01;
        elif t_cmd == 's':
            wpt_control.throttle_cmd = wpt_control.throttle_cmd - 0.01;

        if s_cmd == 'a':
            wpt_control.steer_cmd = wpt_control.steer_cmd + 0.01;
        elif s_cmd == 'd':
            wpt_control.steer_cmd = wpt_control.steer_cmd - 0.01
        wpt_control.publish_command(wpt_control.steer_cmd, wpt_control.throttle_cmd)

        rospy.loginfo("Commands: (steer=%.3f, accel=%.3f). Errors: (CrossTrackError=%.3f, YawError=%.3f, SpeedError=%.3f)." %(wpt_control.steer_cmd, wpt_control.throttle_cmd, error_y, error_yaw, error_v))
